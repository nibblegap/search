...
$manifestfile =
    $destpath .
    "/" .
    str_replace(":", "_", $initurl2["authority"]) .
    "_manifest.json";
$opsfile =
    $destpath .
    "/" .
    str_replace(":", "_", $initurl2["authority"]) .
    "_ops_" .
    md5(($linkdepth === false ? "-1" : $linkdepth) . "|" . $initurl) .
    ".json";
...
function SetReverseManifestPath($key)
{
    ...
    $path = "";
    if (strcasecmp($url2["authority"], $initurl2["authority"]) != 0) {
        $path .= "/" . str_replace(":", "_", strtolower($url2["authority"]));
    }
    $path .= $url2["path"] !== "" ? $url2["path"] : "/";
    $path = explode(
        "/",
        str_replace("\\", "/", TagFilterStream::MakeValidUTF8($path))
    );
    ...
    while (count($vals)) {
        ...
        if (isset($manifestrev[strtolower($path)])) {
            $path = $manifestrev[strtolower($path)];
        } else {
            $manifestrev[strtolower($path)] = $path;
        }

        $x = 0;
        while (is_file($destpath . $path . ($x ? "_" . ($x + 1) : ""))) {
            $x++;
        }
    ...
    while (
        ...
        is_dir($path . ($x ? "_" . ($x + 1) : "") . $ops[$key]["ext"])
    ) {
        $x++;
    }
...
function MapManifestResourceItem($parenturl, $url)
{
    ...
    if (
        $parenturl !== false &&
        isset($opsdata[$parenturl]) &&
        (isset($manifest[$url2]) || isset($opsdata[$url]))
    ) {
        $path = explode("/", $opsdata[$parenturl]["path"]);
        $path2 = explode(
            "/",
            isset($manifest[$url2]) ? $manifest[$url2] : $opsdata[$url]["path"]
        );
...
function PrepareManifestResourceItem($parenturl, $forcedext, $url)
{
    ...
    if (!isset($ops[$key])) {
        $ops[$key] = [
            ...
            "web" =>
                $parenturl === false
                    ? new WebBrowser(["followlocation" => false])
                    : clone $ops[$parenturl]["web"],
            "options" => [
                "pre_retrievewebpage_callback" => "DisplayURL",
            ],
        ];

        $ops[$key]["web"]->ProcessAsync(
            $helper,
            $key,
            null,
            $url,
    ...
    if ($parenturl !== false) {
        if ($ops[$parenturl]["status"] === "waiting") {
            $ops[$parenturl]["wait_refs"]++;
        } else {
...
function ProcessCSS($css, $parenturl, $baseurl)
{
    ...
    while (($pos = stripos($css, "@import", $pos)) !== false) {
        ...
        if ($pos2 < $semipos && $pos3 < $semipos) {
            $url = HTTP::ConvertRelativeToAbsoluteURL(
                $baseurl,
                substr($css, $pos2 + 1, $pos3 - $pos2 - 1)
            );

            $result = str_replace(
                substr($css, $pos2, $pos3 - $pos2 + 1),
                $css[$pos2] .
                    PrepareManifestResourceItem($parenturl, ".css", $url) .
                    $css[$pos2],
                $result
            );
        }
    ...
    while (($pos = stripos($css, "url(", $pos)) !== false) {
        ...
        $result = str_replace(
            substr($css, $pos2, $pos3 - $pos2 + 1),
            $css[$pos2] .
                PrepareManifestResourceItem($parenturl, false, $url) .
                $css[$pos3],
            $result
        );
...
function ProcessContent($key, $final)
{
    ...
    if ($ops[$key]["type"] === "node" && $ops[$key]["ext"] === ".html") {
        ...
        foreach ($rows as $row) {
            ...
            if (isset($row->srcset)) {
                ...
                foreach ($urls as $url) {
                    ...
                    $pos = strrpos($url, " ");
                    if ($pos !== false) {
                        $url2 = HTTP::ConvertRelativeToAbsoluteURL(
                            $urlinfo,
                            trim(substr($url, 0, $pos))
                        );
                        $size = substr($url, $pos + 1);

                        $urls2[] =
                            PrepareManifestResourceItem($key, false, $url2) .
                            " " .
                            $size;
                    }
                }
        ...
        foreach ($rows as $row) {
            $url = HTTP::ConvertRelativeToAbsoluteURL(
                $urlinfo,
                isset($row->href) ? $row->href : $row->{"xlink:href"}
            );

            $row->href = PrepareManifestResourceItem(
                $key,
                (isset($row->rel) && strtolower($row->rel) === "stylesheet") ||
                (isset($row->type) && strtolower($row->type) === "text/css")
                    ? ".css"
                    : false,
                $url
            );
        ...
        foreach ($rows as $row) {
            ...
            if (
                strcasecmp($url2["authority"], $initurl2["authority"]) == 0 &&
                ($url2["scheme"] === "http" || $url2["scheme"] === "https")
            ) {
                if ($url2["path"] === "") {
                    $url2["path"] = "/";
        ...
        if ($final) {
            $opsdata[$key]["content"] = $root->GetOuterHTML();
        }
    }
...
function SaveQueues()
{
    ...
    file_put_contents(
        $opsfile,
        json_encode($ops2, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT)
    );
}
    ...
    foreach ($result["removed"] as $key => $info) {
        if (!$info["result"]["success"]) {
            ...
            echo "Error retrieving URL (" .
                $key .
                ").  " .
                ($ops[$key]["retries"] > 0 ? "Retrying in a moment.  " : "") .
                $info["result"]["error"] .
                " (" .
                $info["result"]["errorcode"] .
                ")\n";
        } else {
            echo "[" .
                number_format(count($ops), 0) .
            ...
            if ($ops[$key]["ext"] === false) {
                $ops[$key]["ext"] = GetResultFileExtension($info["result"]);
            }
            ...
            while (count($process)) {
                ...
                if ($opsdata[$key2]["httpcode"] >= 400) {
                    ...
                    $manifest[
                        str_replace(["http://", "https://"], "//", $key2)
                    ] = $opsdata[$key2]["path"];

                    // Write data to disk.
                    file_put_contents(
                        $destpath . $opsdata[$key2]["path"],
                        $opsdata[$key2]["content"]
                    );
                }
