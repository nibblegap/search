...
class DOHWebBrowser extends WebBrowser
{
    ...
    public function __construct($prevstate = [])
    {
        ...
        if (!is_array(self::$dohcache)) {
            self::$dohcache = [];
        }
    }

    public function SetDOHAccessInfo(
        $dohapi,
        $dohhost = false,
        $dohtypes = ["A", "AAAA"]
    ) {
    ...
    public function ProcessState(&$state)
    {
        if (!isset($state["tempoptions"]["doh_pre_retrievewebpage_callback"])) {
            $state["tempoptions"]["doh_pre_retrievewebpage_callback"] =
                isset($state["tempoptions"]["pre_retrievewebpage_callback"]) &&
                is_callable(
    ...
    public function InternalDNSOverHTTPSHandler(&$state)
    {
        ...
        if (strpos($host, ":") === false && !preg_match('/^[0-9.]+$/', $host)) {
            if (
                isset(self::$dohcache[$host]) &&
                self::$dohcache[$host]["expires"] < time()
            ) {
                unset(self::$dohcache[$host]);
            }

            // Obtain the IP address to connect to and cache it for later reuse.
            if (!isset(self::$dohcache[$host])) {
                if ($this->dohweb === false) {
                    $this->dohweb = new WebBrowser();
                }
                ...
                foreach ($this->dohtypes as $type) {
                    ...
                    if ($this->dohhost !== false) {
                        $options["headers"]["Host"] = $this->dohhost;
                    }
                    if ($this->dohfp !== false) {
                        $options["fp"] = $this->dohfp;
                    }
                    ...
                    if (
                        ...
                        if (
                            ...
                            self::$dohcache[$host]["expires"] =
                                time() + self::$dohcache[$host]["TTL"];
        ...
        if (
            $state["options"]["doh_pre_retrievewebpage_callback"] !== false &&
            !call_user_func_array(
                $state["options"]["doh_pre_retrievewebpage_callback"],
                [&$state]
            )
        ) {
            return false;
