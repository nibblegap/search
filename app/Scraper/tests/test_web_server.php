...
function ProcessAPI($url, $data)
{
    if (!is_array($data)) {
        return [
            "success" => false,
            "error" =>
                "Data sent is not an array/object or was not able to be decoded.",
            "errorcode" => "invalid_data",
        ];
    }

    if (!isset($data["pre"]) || !isset($data["op"]) || !isset($data["post"])) {
        return [
            "success" => false,
            "error" => "Missing required 'pre', 'op', or 'post' data entries.",
            "errorcode" => "missing_data",
        ];
    }

    // Sanitize inputs.
    $data["pre"] = (float) $data["pre"];
    $data["op"] = (string) $data["op"];
    $data["post"] = (float) $data["post"];
...
do {
    ...
    foreach ($result["clients"] as $id => $client) {
        if ($client->appdata === false) {
            ...
            $client->appdata = ["validapikey" => false];
        }

        // Example of checking for an API key.
        if (
            !$client->appdata["validapikey"] &&
            isset($client->requestvars["apikey"]) &&
            $client->requestvars["apikey"] === "123456789101112"
        ) {
        ...
        if ($client->requestcomplete) {
            if (!$client->appdata["validapikey"]) {
                ...
                $client->AddResponseContent(
                    json_encode([
                        "success" => false,
                        "error" => "Invalid or missing 'apikey'.",
                        "errorcode" => "invalid_missing_apikey",
                    ])
                );
                $client->FinalizeResponse();
            } else {
                // Handle WebSocket upgrade requests.
                $id2 = $wsserver->ProcessWebServerClientUpgrade(
                    $webserver,
                    $client
                );
                if ($id2 !== false) {
                    echo "Client ID " .
                        $id .
                        " upgraded to WebSocket.  WebSocket client ID is " .
                        $id2 .
                        ".\n";
                } else {
                    ...
                    } elseif (!is_object($client->readdata)) {
                        $data = @json_decode($client->readdata, true);
                    } else {
                        $client->readdata->Open();
                        $data = @json_decode(
                            $client->readdata->Read(1000000),
                            true
                        );
                    }
