...
$web = new WebBrowser([
    "extractforms" => true,
    "httpopts" => [
        "pre_retrievewebpage_callback" => "DisplayInteractiveRequest",
    ],
]);
...
do {
    ...
    if (!$result2["success"]) {
        ...
        $url = false;
    } elseif ($result2["response"]["code"] != 200) {
        echo "An unexpected response code was returned.  " .
            $result2["response"]["line"] .
            "\n";
        ...
        foreach ($links as $num => $info) {
            echo "\t" .
                ($num + 1) .
                ":  " .
                ($info["display"] !== "" ? $info["display"] : $info["url"]) .
                "\n";
        }
        echo "\n";

        if (count($result2["forms"])) {
            echo "Available forms:  " . count($result2["forms"]) . "\n\n";
        }
        ...
        do {
            ...
            $cmd != "exit" &&
            strpos($cmd, "/") === false &&
            !isset($links[(int) $cmd - 1])
        );

        if ($cmd === "exit") {
            exit();
        } elseif ($cmd === "forms") {
            $result = $web->InteractiveFormFill($result2["forms"]);
