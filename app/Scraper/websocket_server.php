...
class WebSocketServer
{
    ...
    protected $defaultclosemode,
        $defaultmaxreadframesize,
        $defaultmaxreadmessagesize,
        $defaultkeepalive,
        $lasttimeoutcheck;
    ...
    {
        $this->defaultmaxreadframesize = is_bool($maxsize)
            ? false
            : (int) $maxsize;
    }
    ...
    {
        $this->defaultmaxreadmessagesize = is_bool($maxsize)
            ? false
            : (int) $maxsize;
    }
    ...
    protected function ProcessNewConnection($method, $path, $client)
    {
        ...
        if ($method !== "GET") {
            ...
            !isset($client->headers["Connection"]) ||
            stripos($client->headers["Connection"], "upgrade") === false ||
            !isset($client->headers["Upgrade"]) ||
            stripos($client->headers["Upgrade"], "websocket") === false ||
            !isset($client->headers["Sec-Websocket-Key"])
        ) {
            $result .= "HTTP/1.1 400 Bad Request\r\nConnection: close\r\n\r\n";
        } elseif (
            !isset($client->headers["Sec-Websocket-Version"]) ||
            $client->headers["Sec-Websocket-Version"] != 13
        ) {
            $result .=
                "HTTP/1.1 426 Upgrade Required\r\nSec-WebSocket-Version: 13\r\nConnection: close\r\n\r\n";
        } elseif (
            !isset($client->headers["Origin"]) ||
            ($this->origins !== false &&
                !isset($this->origins[strtolower($client->headers["Origin"])]))
        ) {
    ...
    private function ProcessInitialResponse($method, $path, $client)
    {
        ...
        $client->writedata .= $this->ProcessNewConnection(
            $method,
            $path,
            $client
        );
        ...
        if ($client->writedata === "") {
            ...
            $client->writedata .=
                ...
                "\r\n";
            $client->writedata .= $this->ProcessAcceptedConnection(
                $method,
                $path,
                $client
            );
            ...
            $client->websocket->SetMaxReadFrameSize(
                $this->defaultmaxreadframesize
            );
            $client->websocket->SetMaxReadMessageSize(
                $this->defaultmaxreadmessagesize
            );

            // Set the socket in the WebSocket class.
            $client->websocket->Connect("", "", [
                "connected_fp" => $client->fp,
            ]);
        }
    ...
    protected function ProcessWaitResult(&$result)
    {
        ...
        foreach ($result["readfps"] as $cid => $fp) {
            ...
            if ($client->websocket !== false) {
                $this->ProcessClientQueuesAndTimeoutState(
                    $result,
                    $id,
                    true,
                    isset($result["writefps"][$cid])
                );
                        ...
                        while (
                            ...
                            if ($client->request === false) {
                                ...
                                if (
                                    ...
                                    $client->lastheader = self::HeaderNameCleanup(
                                        substr($line, 0, $pos)
                                    );
                                    $client->headers[
                                        $client->lastheader
                                    ] = ltrim(substr($line, $pos + 1));
                                }
                            } else {
        ...
        foreach ($result["writefps"] as $cid => $fp) {
            ...
            if ($client->writedata === "") {
                ...
                if ($result2 === false || ($result2 === "" && feof($fp))) {
                    ...
                    $client->writedata = (string) substr(
                        $client->writedata,
                        $result2
                    );
    ...
    protected function ProcessClientQueuesAndTimeoutState(
        &$result,
        $id,
        $read,
        $write,
        ...
        $result2 = $client->websocket->ProcessQueuesAndTimeoutState(
            $read,
            $write,
            $readsize
        );
