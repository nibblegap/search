...
if (!class_exists("WebSocketServer", false)) {
...
class LibEvWebSocketServer extends WebSocketServer
{
    ...
    {
        if ($revents & Ev::READ) {
            $this->ev_read_ready[$watcher->data] = $watcher->fd;
        }
        if ($revents & Ev::WRITE) {
            $this->ev_write_ready[$watcher->data] = $watcher->fd;
        }
    }
    ...
    public function Start($host, $port)
    {
        ...
        $this->ev_watchers["ws_s"] = new EvIo(
            $this->fp,
            Ev::READ,
            [$this, "Internal_LibEvHandleEvent"],
            "ws_s"
        );
    ...
    protected function InitNewClient($fp)
    {
        ...
        $this->ev_watchers["ws_c_" . $client->id] = new EvIo(
            $client->fp,
            Ev::READ,
            [$this, "Internal_LibEvHandleEvent"],
            "ws_c_" . $client->id
    ...
    public function Wait(
        ...
        $tempwatchers[] = new EvTimer($timeout, 0, [
            $this,
            "Internal_LibEvTimeout",
        ]);
        ...
        foreach ($tempwatchers as $watcher) {
            $watcher->stop();
        }
        ...
        $result["exceptfps"] = is_array($exceptfps) ? [] : $exceptfps;
