...
class WebServer
{
    ...
    public function Reset()
    {
        ...
        $this->initclients = [];
        $this->clients = [];
        $this->readreadyclients = [];
        $this->writereadyclients = [];
        $this->nextclientid = 1;
    ...
    {
        if (!class_exists("DeflateStream", false)) {
            require_once str_replace("\\", "/", dirname(__FILE__)) .
                "/deflate_stream.php";
        }
    ...
    public function Start($host, $port, $sslopts = false)
    {
        ...
        if ($this->fp === false) {
            return [
                "success" => false,
                "error" => HTTP::HTTPTranslate(
                    "Bind() failed.  Reason:  %s (%d)",
                    $errorstr,
                    $errornum
                ),
    ...
    public function AddClientRecvHeader($id, $name, $val)
    {
        ...
        if (substr($name, -2) !== "[]") {
            ...
            if (
                !isset($client->requestvars[$name]) ||
                !is_array($client->requestvars[$name])
            ) {
                $client->requestvars[$name] = [];
            }
            $client->requestvars[$name][] = $val;
        }
    }
    ...
    public function ProcessClientRequestHeaders($request, $headers, $id)
    {
        ...
        if (isset($client->headers["Content-Type"])) {
            $client->contenttype = HTTP::ExtractHeader(
                $client->headers["Content-Type"]
            );
        }

        if (isset($client->headers["Host"])) {
            $client->headers["Host"] = preg_replace(
                "/[^a-z0-9.:\[\]_-]/",
                "",
                strtolower($client->headers["Host"])
            );
        }
        ...
        if (isset($client->headers["Cookie"])) {
            ...
            foreach ($cookies as $cookie) {
                ...
                if (substr($name, -2) !== "[]") {
                    ...
                    if (
                        !isset($client->cookievars[$name]) ||
                        !is_array($client->cookievars[$name])
                    ) {
                        $client->cookievars[$name] = [];
                    }
                    $client->cookievars[$name][] = $val;
                }
            }
    ...
    public function ProcessClientRequestBody($request, $body, $id)
    {
        ...
        if ($body !== "") {
            if (is_object($client->readdata)) {
                $client->readdata->Write($body);
            } else {
                $client->readdata .= $body;
                if (strlen($client->readdata) > 262144) {
                    return false;
                }
                ...
                if ($client->contenttype !== false) {
                    if (
                        ...
                        isset($client->contenttype["boundary"])
                    ) {
                        $pos = 0;
                        do {
                                    ...
                                    while (
                                        ...
                                        if ($header != "") {
                                            if (
                                                ...
                                                $client->lastmimeheader = HTTP::HeaderNameCleanup(
                                                    substr($header, 0, $pos2)
                                                );
                                                $client->mimeheaders[
                                                    $client->lastmimeheader
                                                ] = ltrim(
                                                    substr($header, $pos2 + 1)
                                                );
                                            }
                                            ...
                                            if (
                                                ...
                                                $client->mode =
                                                    "handle_request_mime_content_skip";
                                            } else {
                                                $client->mime_contentdisposition = HTTP::ExtractHeader(
                                                    $client->mimeheaders[
                                                        "Content-Disposition"
                                                    ]
                                                );
                                                ...
                                                if (
                                                    ...
                                                    $client->mode =
                                                        "handle_request_mime_content_file";
                                                } else {
                                                    $client->mime_value = "";

                                                    $client->mode =
                                                        "handle_request_mime_content";
                                                }
                                            }
    ...
    public function ProcessClientResponseBody(&$data, &$bodysize, $id)
    {
        ...
        if ($bodysize === false && $client->responsefinalized) {
            $bodysize = true;
        }
    ...
    public function UpdateStreamsAndTimeout(
        $prefix,
        &$timeout,
        &$readfps,
        &$writefps
        ...
        if (count($this->readreadyclients)) {
            $timeout = 0;
        }
        ...
        foreach ($this->clients as $id => $client) {
            if ($client->httpstate !== false) {
                if (
                    $ts < $client->httpstate["waituntil"] &&
                    $timeout > $client->httpstate["waituntil"] - $ts + 0.5
                ) {
                    $timeout =
                        (int) ($client->httpstate["waituntil"] - $ts + 0.5);

                    $client->lastts = $ts;
                } elseif (HTTP::WantRead($client->httpstate)) {
                    $readfps[$prefix . "http_c_" . $id] = $client->fp;
                } elseif (
                ...
                } elseif ($client->responsefinalized) {
    ...
    public static function FixedStreamSelect(
        &$readfps,
        &$writefps,
        &$exceptfps,
        $timeout
        ...
        if (
            isset($readfps[0]) ||
            isset($writefps[0]) ||
            ($exceptfps !== null && isset($exceptfps[0]))
        ) {
            return false;
        }
    ...
    public function Wait(
        ...
        $result2 = self::FixedStreamSelect(
            $readfps,
            $writefps,
            $exceptfps,
            $timeout
        ...
        foreach ($this->readreadyclients as $id => $fp) {
            $readfps["http_c_" . $id] = $fp;
        }
        $this->readreadyclients = [];
        ...
        foreach ($readfps as $cid => $fp) {
            ...
            if ($client->httpstate !== false) {
                ...
                if ($result2["success"]) {
                    ...
                    if (
                        $this->usegzip &&
                        isset($client->headers["Accept-Encoding"])
                    ) {
                        $encodings = HTTP::ExtractHeader(
                        ...
                        if ($gzip) {
                            ...
                            $client->deflate->Init("wb", -1, [
                                "type" => "gzip",
                            ]);

                            $client->AddResponseHeader(
                                "Content-Encoding",
                                "gzip",
                                true
                            );
                ...
                } elseif ($result2["errorcode"] !== "no_data") {
                    if ($client->requests) {
                        $result["removed"][$id] = [
                            "result" => $result2,
                            "client" => $client,
                    ...
                    $client->requestcomplete === false &&
                    $client->httpstate["state"] !== "request_line" &&
                    $client->httpstate["state"] !== "headers"
                ) {
        ...
        // Handle ready clients.
        foreach ($this->writereadyclients as $id => $fp) {
            $writefps["http_c_" . $id] = $fp;
        }
        $this->writereadyclients = [];
                ...
                if ($client->mode === "response_ready") {
                    if ($client->responsefinalized) {
                        if ($client->responsebodysize !== false) {
                            $client->AddResponseHeader(
                                "Content-Length",
                                (string) strlen($client->writedata),
                                true
                            );
                        }
                        $client->httpstate["bodysize"] = strlen(
                            $client->writedata
                        );
                    } elseif ($client->responsebodysize !== true) {
                        $client->AddResponseHeader(
                            "Content-Length",
                            (string) $client->responsebodysize,
                            true
                        );
                        $client->httpstate["bodysize"] =
                            $client->responsebodysize;
                    } elseif ($client->keepalive) {
                        $client->AddResponseHeader(
                            "Transfer-Encoding",
                            "chunked",
                            true
                    ...
                    if (
                        !$client->keepalive ||
                        $client->requests >= $this->maxrequests
                    ) {
                    ...
                    $client->httpstate["data"] .= "\r\n";
                    $client->httpstate["result"]["rawsendheadersize"] = strlen(
                        $client->httpstate["data"]
                    );
                ...
                switch ($client->mode) {
                    ...
                    case "init_request":
                        ...
                        $options["read_headers_callback"] = [
                            $this,
                            "ProcessClientRequestHeaders",
                        ];
                        $options["read_headers_callback_opts"] = $id;
                        $options["read_body_callback"] = [
                            $this,
                            "ProcessClientRequestBody",
                        ];
                        $options["read_body_callback_opts"] = $id;
                        $options["write_body_callback"] = [
                            $this,
                            "ProcessClientResponseBody",
                        ];
                        ...
                        $client->httpstate = HTTP::InitResponseState(
                            $client->fp,
                            $debug,
                            $options,
                            $startts,
            ...
            if ($client->lastts + $this->defaultclienttimeout < $ts) {
                if ($client->requests) {
                    $result2 = [
                        "success" => false,
                        "error" => HTTP::HTTPTranslate(
                            "Client timed out.  Most likely cause:  Connection failure."
                        ),
...
class WebServer_Client
{
    ...
    {
        if ($this->httpstate !== false) {
            $this->httpstate["options"] = $options;
        }
    }
    ...
    public function SetResponseCode($code)
    {
        if ($this->requestcomplete && $this->mode !== "handle_response") {
            if (is_int($code)) {
                ...
                if (
                    $this->responsebodysize === true &&
                    (($code >= 100 && $code < 200) ||
                        $code === 204 ||
                        $code === 304)
                ) {
                    $this->responsebodysize = false;
                }
    ...
    public function SetResponseCookie(
        ...
        if (!empty($domain)) {
            ...
            $pos = strpos($domain, "]");
            if (substr($domain, 0, 1) == "[" && $pos !== false) {
    ...
    public function AddResponseHeader($name, $val, $replace = false)
    {
        if ($this->requestcomplete && $this->mode !== "handle_response") {
            ...
            if (!isset($this->responseheaders[$name]) || $replace) {
                $this->responseheaders[$name] = [];
            }
            $this->responseheaders[$name][] = str_replace(
                ["\r", "\n"],
                ["", ""],
                $val
            );
    ...
    public function AddResponseContent($data)
    {
        if ($this->requestcomplete && !$this->responsefinalized) {
            ...
            if ($this->mode !== "handle_response") {
                $this->mode = "response_ready";
            }
        }
    }
