...
class WebBrowser
{
        ...
        while ($state["state"] !== "done") {
            switch ($state["state"]) {
                case "initialize":
                    if (
                        !isset(
                            $this->data["allowedprotocols"][
                                $state["urlinfo"]["scheme"]
                            ]
                        ) ||
                        !$this->data["allowedprotocols"][
                            $state["urlinfo"]["scheme"]
                        ...
                        return [
                            "success" => false,
                            "error" => self::WBTranslate(
                                "Protocol '%s' is not allowed in '%s'.",
                                $state["urlinfo"]["scheme"],
                                $state["url"]
                            ),
                    ...
                    $filename = HTTP::ExtractFilename(
                        $state["urlinfo"]["path"]
                    );
                    $pos = strrpos($filename, ".");
                    $fileext =
                        $pos !== false
                            ? strtolower(substr($filename, $pos + 1))
                            : "";
                    ...
                    if (
                        substr($profile, 0, 2) == "ie" ||
                        ($profile == "auto" &&
                            substr($this->data["useragent"], 0, 2) == "ie")
                    ) {
                        if ($fileext == "css") {
                        ...
                        $headers["User-Agent"] = HTTP::GetUserAgent(
                            substr($profile, 0, 2) == "ie"
                                ? $profile
                                : $this->data["useragent"]
                        );
                        ...
                        $profile == "chrome" ||
                        ($profile == "auto" &&
                            ($this->data["useragent"] == "safari" ||
                                $this->data["useragent"] == "chrome"))
                    ) {
                        if ($fileext == "css") {
                            $headers["Accept"] = "text/css,*/*;q=0.1";
                        } elseif (
                    ...
                    $headers = array_merge(
                        $headers,
                        $state["httpopts"]["headers"],
                        $state["tempoptions"]["headers"]
                    );

                    // Calculate the host and reverse host and remove port information.
                    $host = isset($headers["Host"])
                        ? $headers["Host"]
                        : $state["urlinfo"]["host"];
                    $pos = strpos($host, "]");
                    ...
                    if (isset($headers["Authorization"])) {
                        $this->data["hostauths"][$host] =
                            $headers["Authorization"];
                    } elseif (isset($this->data["hostauths"][$host])) {
                        $headers["Authorization"] =
                            $this->data["hostauths"][$host];
                    }
                    ...
                    foreach ($this->data["cookies"] as $domain => $paths) {
                        if (
                            strlen($dothost) >= strlen($domain) &&
                            substr($dothost, -strlen($domain)) === $domain
                        ) {
                            foreach ($paths as $path => $cookies2) {
                                if (
                                    substr($cookiepath, 0, strlen($path)) ==
                                    $path
                                ) {
                                    foreach ($cookies2 as $num => $info) {
                                        if (
                                            isset($info["expires_ts"]) &&
                                            $this->GetExpiresTimestamp(
                                                $info["expires_ts"]
                                            ) < time()
                                        ) {
                                            unset(
                                                $this->data["cookies"][$domain][
                                                    $path
                                                ][$num]
                                            );
                                        } elseif (
                    ...
                    $state["options"] = array_merge(
                        $state["httpopts"],
                        $state["tempoptions"]
                    );
                    $state["options"]["headers"] = $headers;
                    if ($state["timeout"] !== false) {
                        $state["options"]["timeout"] = HTTP::GetTimeLeft(
                            $state["startts"],
                            $state["timeout"]
                        );
                    }
                    ...
                    if (
                        isset(
                            $state["options"]["pre_retrievewebpage_callback"]
                        ) &&
                        is_callable(
                            $state["options"]["pre_retrievewebpage_callback"]
                        ) &&
                        ...
                        return [
                            "success" => false,
                            "error" => self::WBTranslate(
                                "Pre-RetrieveWebpage callback returned with a failure condition for '%s'.",
                                $state["url"]
                            ),
                            "errorcode" => "pre_retrievewebpage_callback",
                        ];
                    }

                    // Process the request.
                    $result = HTTP::RetrieveWebpage(
                        $state["url"],
                        $state["options"]
                    );
                    ...
                    if (isset($result["rawsendsize"])) {
                        $state["totalrawsendsize"] += $result["rawsendsize"];
                    }
                ...
                case "post_retrieval":
                    ...
                    if (
                        isset($state["result"]["headers"]["Location"]) &&
                        $this->data["followlocation"]
                    ) {
                        ...
                        $state["tempoptions"]["headers"]["Referer"] =
                            $state["url"];
                        $state["url"] =
                            $state["result"]["headers"]["Location"][0];

                        // Generate an absolute URL.
                        if ($this->data["referer"] != "") {
                            $state["url"] = HTTP::ConvertRelativeToAbsoluteURL(
                                $this->data["referer"],
                                $state["url"]
                            );
                        }
                        ...
                        if (
                            !isset(
                                $this->data["allowedredirprotocols"][
                                    $urlinfo2["scheme"]
                                ]
                            ) ||
                            !$this->data["allowedredirprotocols"][
                            ...
                            return [
                                "success" => false,
                                "error" => self::WBTranslate(
                                    "Protocol '%s' is not allowed.  Server attempted to redirect to '%s'.",
                                    $urlinfo2["scheme"],
                                    $state["url"]
                                ),
                                "info" => $state["result"],
                        ...
                        if ($urlinfo2["host"] != $state["urlinfo"]["host"]) {
                            ...
                            unset(
                                $state["httpopts"]["headers"]["Authorization"]
                            );
                            unset(
                                $state["tempoptions"]["headers"][
                                    "Authorization"
                    ...
                    if (isset($state["result"]["headers"]["Set-Cookie"])) {
                        foreach (
                            $state["result"]["headers"]["Set-Cookie"]
                            as $cookie
                        ) {
                            ...
                            if ($item != "") {
                                ...
                                if ($pos === false) {
                                    ...
                                    $cookie2["name"] = urldecode(
                                        substr($item, 0, $pos)
                                    );
                                    $cookie2["value"] = urldecode(
                                        substr($item, $pos + 1)
                                    );
                                }
                                ...
                                if (isset($cookie["expires"])) {
                                    $ts = HTTP::GetDateTimestamp(
                                        $cookie["expires"]
                                    );
                                    $cookie["expires_ts"] = gmdate(
                                        "Y-m-d H:i:s",
                                        $ts === false
                                            ? time() - 24 * 60 * 60
                                            : $ts
                                ...
                                if (!isset($cookie["domain"])) {
                                    $cookie["domain"] = $state["dothost"];
                                }
                                if (!isset($cookie["path"])) {
                                    $cookie["path"] = $state["cookiepath"];
                                }
                    ...
                    if (
                        ...
                        if ($this->data["extractforms"]) {
                            $state["result"]["forms"] = $this->ExtractForms(
                                $state["result"]["url"],
                                $state["result"]["body"],
                                isset(
                                    $state["tempoptions"]["extractforms_hint"]
                                )
                                    ? $state["tempoptions"]["extractforms_hint"]
                                    : false
                            );
    ...
    public function Process($url, $tempoptions = [])
    {
        ...
        $profile = isset($tempoptions["profile"])
            ? $tempoptions["profile"]
            : "auto";
        ...
        } elseif (isset($this->data["httpopts"]["streamtimeout"])) {
        ...
        $this->data["httpopts"]["headers"] = HTTP::NormalizeHeaders(
            $this->data["httpopts"]["headers"]
        );
        ...
        $tempoptions["headers"] = HTTP::NormalizeHeaders(
            $tempoptions["headers"]
        );
        ...
        if ($this->data["referer"] != "") {
            $url = HTTP::ConvertRelativeToAbsoluteURL(
                $this->data["referer"],
                $url
            );
        }
    ...
    public function ProcessAsync__Handler($mode, &$data, $key, &$info)
    {
        switch ($mode) {
                ...
                if ($info["init"]) {
                    ...
                    if (!$info["result"]["success"]) {
                        ...
                        if (is_callable($info["callback"])) {
                            call_user_func_array($info["callback"], [
                                $key,
                                $info["url"],
                                $info["result"],
                            ]);
            ...
            case "readfps":
                if (
                    $info["state"]["httpstate"] !== false &&
                    HTTP::WantRead($info["state"]["httpstate"])
                ) {
            ...
            case "writefps":
                if (
                    $info["state"]["httpstate"] !== false &&
                    HTTP::WantWrite($info["state"]["httpstate"])
                ) {
                    $data[$key] = $info["state"]["httpstate"]["fp"];
                }
                ...
                if ($data === true) {
                    if (isset($info["state"])) {
                        if ($info["state"]["httpstate"] !== false) {
                            HTTP::ForceClose($info["state"]["httpstate"]);
                        }
    ...
    public function ProcessAsync(
        ...
        $helper->Set($key, $info, [$this, "ProcessAsync__Handler"]);
    ...
    public function ExtractForms($baseurl, $data, $hint = false)
    {
        ...
        $html5rows = $this->html->find(
            "input[form],textarea[form],select[form],button[form],datalist[id]" .
                ($hint !== false ? "," . $hint : "")
        );
        ...
        foreach ($rows as $row) {
            ...
            $info["action"] = isset($row->action)
                ? HTTP::ConvertRelativeToAbsoluteURL(
                    $baseurl,
                    (string) $row->action
                )
                : $baseurl;
            $info["method"] =
                isset($row->method) && strtolower(trim($row->method)) == "post"
                    ? "post"
                    : "get";
            if ($info["method"] == "post") {
            ...
            foreach ($rows2 as $row2) {
                if (!isset($row2->form)) {
                    if (
                        isset($row2->id) &&
                        $row2->id != "" &&
                        isset($hintmap[trim($row2->id)])
                    ) {
                        $lasthint = $hintmap[trim($row2->id)];
                    }
    ...
    private function ExtractFieldFromDOM(&$fields, $row, &$lasthint)
    {
        switch ($row->tag) {
            case "input":
                if (
                    !isset($row->name) &&
                    ($row->type === "submit" || $row->type === "image")
                ) {
                    $row->name = "";
                }
                ...
                if (isset($row->name) && is_string($row->name)) {
                    ...
                    if (isset($row->placeholder)) {
                        $field["hint"] = trim($row->placeholder);
                    } elseif (
                        $field["type"] == "input.submit" ||
                        $field["type"] == "input.image"
                    ) {
                        $field["hint"] = $field["type"] . "|" . $field["value"];
                    } elseif ($lasthint !== "") {
                        $field["hint"] = $lasthint;
                    }
                ...
                if (isset($row->name) && is_string($row->name)) {
                    if (isset($row->multiple)) {
                        ...
                        foreach ($rows as $row2) {
                            ...
                            if ($val === false && isset($row2->selected)) {
    ...
    public static function InteractiveFormFill($forms, $showselected = false)
    {
        ...
        if (count($forms) == 1) {
            ...
            echo self::WBTranslate(
                "There are multiple forms available to fill out:\n"
            );
            foreach ($forms as $num => $form) {
                ...
                echo self::WBTranslate(
                    "\t\tbuttons:  %d\n",
                    count($form->GetVisibleFields(true)) -
                        count($form->GetVisibleFields(false))
                );
                echo "\n";
            }
        ...
        if (count($form->GetVisibleFields(false))) {
            echo self::WBTranslate(
                "Select form fields by field number to edit a field.  When ready to submit the form, leave 'Field number' empty.\n\n"
            );
            ...
            do {
                ...
                foreach ($form->fields as $num => $field) {
                    if (
                        ...
                        $field["type"] == "input.submit" ||
                        $field["type"] == "input.image" ||
                        $field["type"] == "input.button" ||
                        substr($field["type"], 0, 7) == "button."
                    ) {
                        continue;
                    }
                ...
                do {
                    ...
                    !isset($form->fields[$num]) ||
                    $form->fields[$num]["type"] == "input.hidden" ||
                    $form->fields[$num]["type"] == "input.submit" ||
                    $form->fields[$num]["type"] == "input.image" ||
                    $form->fields[$num]["type"] == "input.button" ||
                    substr($form->fields[$num]["type"], 0, 7) == "button."
                );
        ...
        $submitoptions = [
            [
                "name" => self::WBTranslate("Default action"),
                "value" => self::WBTranslate("Might not work"),
                "hint" => "Default action",
            ],
        ];
        foreach ($form->fields as $num => $field) {
        ...
        if (count($submitoptions) <= 2) {
            $num = count($submitoptions) - 1;
        } else {
        ...
        $result = $form->GenerateFormRequest(
            $num ? $submitoptions[$num]["name"] : false,
            $num ? $submitoptions[$num]["value"] : false
        );
    ...
    public function SetCookie($cookie)
    {
        if (
            ...
            return [
                "success" => false,
                "error" => self::WBTranslate(
                    "SetCookie() requires 'domain', 'path', 'name', and 'value' to be options."
                ),
        ...
        if (
            !isset($this->data["cookies"][$cookie["domain"]][$cookie["path"]])
        ) {
    ...
    public function DeleteCookies($domainpattern, $pathpattern, $namepattern)
    {
        foreach ($this->data["cookies"] as $domain => $paths) {
            if (
                $domainpattern == "" ||
                substr($domain, -strlen($domainpattern)) == $domainpattern
            ) {
                foreach ($paths as $path => $cookies) {
                    if (
                        $pathpattern == "" ||
                        substr($path, 0, strlen($pathpattern)) == $pathpattern
                    ) {
                        foreach ($cookies as $num => $info) {
                            if (
                                $namepattern == "" ||
                                strpos($info["name"], $namepattern) !== false
                            ) {
                                unset(
                                    $this->data["cookies"][$domain][$path][$num]
                                );
                            }
                        }
...
class WebBrowserForm
{
    ...
    public function FindFormFields($name = false, $value = false, $type = false)
    {
        ...
        foreach ($this->fields as $num => $field) {
            if (
                ($type === false || $field["type"] === $type) &&
                ($name === false || $field["name"] === $name) &&
                ($value === false || $field["value"] === $value)
            ) {
                $fields[] = $field;
            }
        }
        ...
        foreach ($this->fields as $num => $field) {
            if (
                ...
                if ($field["type"] == "input.radio") {
                    $this->fields[$num]["checked"] =
                        $field["value"] === $value ? $checked : false;
                    $result = true;
                } elseif ($field["type"] == "input.checkbox") {
    ...
    public function GenerateFormRequest(
        ...
        foreach ($this->fields as $field) {
            if ($field["type"] == "input.file") {
                ...
                $field["type"] == "input.reset" ||
                $field["type"] == "button.reset"
            ) {
            } elseif (
                $field["type"] == "input.submit" ||
                $field["type"] == "input.image" ||
                ...
                if (
                    ($submitname === false || $field["name"] === $submitname) &&
                    ($submitvalue === false || $field["value"] === $submitvalue)
                ) {
                    if ($submitname !== "") {
