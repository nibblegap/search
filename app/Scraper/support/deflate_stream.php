...
class DeflateStream
{
    ...
    public static function IsSupported()
    {
        if (!is_bool(self::$supported)) {
            self::$supported =
                function_exists("stream_filter_append") &&
                function_exists("stream_filter_remove") &&
    ...
    public function Init($mode, $compresslevel = -1, $options = [])
    {
        ...
        if (
            $options["type"] != "zlib" &&
            $options["type"] != "gzip" &&
            ($this->compress || $options["type"] != "auto")
        ) {
            $options["type"] = "raw";
        }
        ...
        if ($this->compress) {
            if ($this->options["type"] == "zlib") {
                ...
                if (isset($this->options["filename"])) {
                    $this->outdata .=
                        str_replace("\x00", " ", $this->options["filename"]) .
                        "\x00";
                }
                if (isset($this->options["comment"])) {
                    $this->outdata .=
                        str_replace("\x00", " ", $this->options["comment"]) .
                        "\x00";
                }
            }
        } else {
    ...
    public function Write($data)
    {
        ...
        if ($this->compress) {
            if ($this->options["type"] == "zlib") {
                ...
                for ($x = 0; $x < $y; $x++) {
                    $this->options["a"] =
                        ($this->options["a"] + ord($data[$x])) % 65521;
                    $this->options["b"] =
                        ($this->options["b"] + $this->options["a"]) % 65521;
                }
            } elseif ($this->options["type"] == "gzip") {
                $this->options["crc32"]->AddData($data);
                $this->options["bytes"] = $this->ADD32(
                    $this->options["bytes"],
                    strlen($data)
                );
            }
    ...
    public function Finalize()
    {
        ...
        if ($this->compress) {
            if ($this->options["type"] == "zlib") {
                $this->outdata .= pack(
                    "N",
                    $this->SHL32($this->options["b"], 16) | $this->options["a"]
                );
            } elseif ($this->options["type"] == "gzip") {
                $this->outdata .=
                    pack("V", $this->options["crc32"]->Finalize()) .
                    pack("V", $this->options["bytes"]);
            }
        }
        ...
        if ($this->options["type"] == "auto") {
            if (strlen($this->indata) >= 3) {
                ...
                if (substr($this->indata, 0, 3) === "\x1F\x8B\x08") {
                    ...
                    (ord($this->indata[0]) & 0x0f) == 8 &&
                    (ord($this->indata[0]) & 0xf0) >> 4 < 8 &&
                    $zlibtest[1] % 31 == 0
                ) {
