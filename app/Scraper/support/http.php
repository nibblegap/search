...
class HTTP
{
    ...
    public static function ExtractURL($url, $idnahost = true)
    {
        ...
        if ($pos !== false) {
            ...
            foreach ($vars as $var) {
                ...
                $name = urldecode($name);
                if (!isset($result["queryvars"][$name])) {
                    $result["queryvars"][$name] = [];
                }
        ...
        if (substr($url, 0, 2) != "//") {
            ...
            if ($pos !== false) {
                ...
                if ($pos === false) {
                    ...
                    $result["loginusername"] = urldecode(
                        substr($result["login"], 0, $pos)
                    );
                    $result["loginpassword"] = urldecode(
                        substr($result["login"], $pos + 1)
                    );
                }
            }
            ...
            if (substr($url, 0, 1) == "[" && $pos !== false) {
                ...
                if ($idnahost) {
                    ...
                    for (
                        $x = 0;
                        $x < $y && ord($result["host"][$x]) <= 0x7f;
                        $x++
                    );

                    if ($x < $y) {
                        if (!class_exists("UTFUtils", false)) {
    ...
    public static function CondenseURL($data)
    {
        ...
        if (isset($data["host"]) && $data["host"] != "") {
            ...
            if (
                isset($data["loginusername"]) &&
                $data["loginusername"] != "" &&
                isset($data["loginpassword"])
            ) {
                $result .=
                    rawurlencode($data["loginusername"]) .
                    ($data["loginpassword"] != ""
                        ? ":" . rawurlencode($data["loginpassword"])
                        : "") .
                    "@";
    ...
    {
        $relative = is_array($relativeurl)
            ? $relativeurl
            : self::ExtractURL($relativeurl);
    ...
    public static function GetUserAgent($type)
    {
        ...
        if ($type == "ie6") {
            ...
            return "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.61 Safari/537.36";
        }
    ...
    public static function GetSafeSSLOpts(
        $cafile = true,
        $cipherstype = "intermediate"
    ) {
    ...
    public static function GetDateTimestamp($httpdate)
    {
        ...
        $items = explode(
            ...
            preg_replace(
                "/\s+/",
                " ",
                str_replace("-", " ", strtolower($httpdate))
            )
        );
    ...
    public static function HTTPTranslate()
    {
        ...
        return call_user_func_array(
            defined("CS_TRANSLATE_FUNC") && function_exists(CS_TRANSLATE_FUNC)
                ? CS_TRANSLATE_FUNC
                : "sprintf",
            $args
        );
    ...
    private static function ProcessSSLOptions(&$options, $key, $host)
    {
        if (isset($options[$key]["auto_cainfo"])) {
            ...
            if ($cainfo !== false && strlen($cainfo) > 0) {
                $options[$key]["cafile"] = $cainfo;
            } elseif (
                file_exists(
                    str_replace("\\", "/", dirname(__FILE__)) . "/cacert.pem"
                )
            ) {
                $options[$key]["cafile"] =
                    str_replace("\\", "/", dirname(__FILE__)) . "/cacert.pem";
    ...
    public static function InitResponseState(
        $fp,
        $debug,
        $options,
        $startts,
        ...
        $state = [
            ...
            "numheaders" => 0,
            "autodecode" =>
                !isset($options["auto_decode"]) || $options["auto_decode"],
    ...
    private static function ProcessState__InternalRead(
        &$state,
        $size,
        $endchar = false
    ) {
        ...
        do {
            ...
            $size <= $y ||
            ($endchar !== false &&
                strpos($state["nextread"], $endchar) !== false)
        );
    ...
    private static function ProcessState__ReadLine(&$state)
    {
        while (strpos($state["data"], "\n") === false) {
            ...
            if ($data2 === false || $data2 === "") {
                if (feof($state["fp"])) {
                    ...
                    return [
                        "success" => false,
                        "error" => self::HTTPTranslate(
                            "Underlying stream encountered a read error."
                        ),
            ...
            if ($pos === false) {
                ...
                if (self::StreamTimedOut($state["fp"])) {
                    return [
                        "success" => false,
                        "error" => self::HTTPTranslate(
                            "Underlying stream timed out."
                        ),
                        "errorcode" => "stream_timeout_exceeded",
                    ];
                }
            ...
            if (
                $state["timeout"] !== false &&
                self::GetTimeLeft($state["startts"], $state["timeout"]) == 0
            ) {
                return [
                    "success" => false,
            ...
            if (
                isset($state["options"]["readlinelimit"]) &&
                strlen($state["data"]) + $pos >
                    $state["options"]["readlinelimit"]
            ) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate("Read line exceeded limit."),
                    "errorcode" => "read_line_limit_exceeded",
                ];
            }
            ...
            if (
                isset($state["options"]["recvlimit"]) &&
                $state["options"]["recvlimit"] < $state["rawsize"]
            ) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "Received data exceeded limit."
                    ),
                    "errorcode" => "receive_limit_exceeded",
                ];
            }
            if (isset($state["options"]["recvratelimit"])) {
                $state["waituntil"] = self::ProcessRateLimit(
                    $state["rawsize"],
                    $state["recvstart"],
                    $state["options"]["recvratelimit"],
                    $state["async"]
                );
            }

            if (
                isset($state["options"]["debug_callback"]) &&
                is_callable($state["options"]["debug_callback"])
            ) {
                call_user_func_array($state["options"]["debug_callback"], [
                    "rawrecv",
                    $data2,
                    &$state["options"]["debug_callback_opts"],
                ]);
            } elseif ($state["debug"]) {
                $state["rawdata"] .= $data2;
            }
    ...
    private static function ProcessState__ReadBodyData(&$state)
    {
        while ($state["sizeleft"] === false || $state["sizeleft"] > 0) {
            $data2 = self::ProcessState__InternalRead(
                $state,
                $state["sizeleft"] === false || $state["sizeleft"] > 65536
                    ? 65536
                    : $state["sizeleft"]
            );
            ...
            if ($state["sizeleft"] !== false) {
                $state["sizeleft"] -= $tempsize;
            }

            if (
                $state["result"]["response"]["code"] == 100 ||
                !isset($state["options"]["read_body_callback"]) ||
                !is_callable($state["options"]["read_body_callback"])
            ) {
                $state["result"]["body"] .= self::GetDecodedBody(
                    $state["autodecode_ds"],
                    $data2
                );
            } elseif (
                !call_user_func_array($state["options"]["read_body_callback"], [
                    $state["result"][$state["client"] ? "response" : "request"],
                    self::GetDecodedBody($state["autodecode_ds"], $data2),
                    &$state["options"]["read_body_callback_opts"],
                ])
            ) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "Read body callback returned with a failure condition."
                    ),
            ...
            if (isset($state["options"]["recvratelimit"])) {
                ...
                if (microtime(true) < $state["waituntil"]) {
                    return [
                        "success" => false,
                        "error" => self::HTTPTranslate(
                            "Rate limit for non-blocking connection has not been reached."
                        ),
                        "errorcode" => "no_data",
                    ];
                }
    ...
    private static function ProcessState__WriteData(&$state, $prefix)
    {
        if ($state[$prefix . "data"] !== "") {
            ...
            if (
                $state["secure"] &&
                $state["async"] &&
                version_compare(PHP_VERSION, "7.1.4") <= 0
            ) {
                ...
                if ($state["debug"]) {
                    $result = fwrite(
                        $state["fp"],
                        strlen($state[$prefix . "data"]) > 4096
                            ? substr($state[$prefix . "data"], 0, 4096)
                            : $state[$prefix . "data"]
                    );
                } else {
                    $result = @fwrite(
            ...
            if ($result === false || feof($state["fp"])) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "A fwrite() failure occurred.  Most likely cause:  Connection failure."
                    ),
            ...
            $state[$prefix . "data"] = (string) substr(
                $state[$prefix . "data"],
                $result
            );
            ...
            if (isset($state["options"]["sendratelimit"])) {
                $state["waituntil"] = self::ProcessRateLimit(
                    $state["result"]["rawsendsize"],
                    $state["result"]["connected"],
                    $state["options"]["sendratelimit"],
                    $state["async"]
                );
                if (microtime(true) < $state["waituntil"]) {
                    return [
            ...
            if (
                ...
                $state["result"]["rawsend"] .= $data2;
            }

            if ($state["async"] && strlen($state[$prefix . "data"])) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "Non-blocking write did not send all data."
                    ),
                    "errorcode" => "no_data",
                ];
            }
    ...
    public static function ProcessState(&$state)
    {
        ...
        if (
            ...
            return self::CleanupErrorState($state, [
                "success" => false,
                "error" => self::HTTPTranslate("HTTP timeout exceeded."),
                "errorcode" => "timeout_exceeded",
            ]);
        ...
        if ($state["type"] === "request") {
            while ($state["state"] !== "done") {
                switch ($state["state"]) {
                    case "connecting":
                        if (
                            ...
                            if ($result === false || count($exceptfp)) {
                                return self::CleanupErrorState($state, [
                                    "success" => false,
                                    "error" => self::HTTPTranslate(
                                        "A stream_select() failure occurred.  Most likely cause:  Connection failure."
                                    ),
                        ...
                        if (
                            isset($state["options"]["streamtimeout"]) &&
                            $state["options"]["streamtimeout"] !== false &&
                            function_exists("stream_set_timeout")
                        ) {
                            @stream_set_timeout(
                                $state["fp"],
                                $state["options"]["streamtimeout"]
                            );
                        }
                        ...
                        if (
                            ...
                            (($state["useproxy"] && $state["proxysecure"]) ||
                                (!$state["useproxy"] && $state["secure"]))
                        ) {
                            $state["state"] = "connecting_enable_crypto";
                        } else {
                    ...
                    case "connecting_enable_crypto":
                        ...
                        if ($result === false) {
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "A stream_socket_enable_crypto() failure occurred.  Most likely cause:  Connection failure or incompatible crypto setup."
                                ),
                                "errorcode" =>
                                    "stream_socket_enable_crypto_failed",
                            ]);
                        } elseif ($result === true) {
                            $state["state"] = "connection_ready";
                        } else {
                            return [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "Non-blocking enable crypto operation is not complete yet."
                                ),
                                "errorcode" => "no_data",
                            ];
                        }
                    ...
                    case "connection_ready":
                        ...
                        if (function_exists("stream_context_get_options")) {
                            $contextopts = stream_context_get_options(
                                $state["fp"]
                            );
                            if ($state["useproxy"]) {
                                if (
                                    $state["proxysecure"] &&
                                    isset($state["options"]["proxysslopts"]) &&
                                    is_array($state["options"]["proxysslopts"])
                                ) {
                                    if (
                                        isset(
                                            $state["options"][
                                        ...
                                        if (
                                            ...
                                            return [
                                                "success" => false,
                                                "error" => self::HTTPTranslate(
                                                    "Peer certificate callback returned with a failure condition."
                                                ),
                                                "errorcode" =>
                                                    "peer_cert_callback",
                                            ];
                                ...
                                if (
                                    $state["secure"] &&
                                    isset($state["options"]["sslopts"]) &&
                                    is_array($state["options"]["sslopts"])
                                ) {
                                    if (
                                        isset(
                                            $state["options"][
                        ...
                        if ($state["proxyconnect"]) {
                            ...
                            $state["result"]["rawsendproxyheadersize"] = strlen(
                                $state["proxydata"]
                            );
                    ...
                    case "proxy_connect_send":
                        ...
                        $result = self::ProcessState__WriteData(
                            $state,
                            "proxy"
                        );
                        if (!$result["success"]) {
                            return self::CleanupErrorState($state, $result);
                        }

                        // Prepare the state for handling the response from the proxy server.
                        $options2 = [];
                        if (isset($state["options"]["async"])) {
                            $options2["async"] = $state["options"]["async"];
                        }
                        if (isset($state["options"]["recvratelimit"])) {
                            $options2["recvratelimit"] =
                                $state["options"]["recvratelimit"];
                        }
                        if (isset($state["options"]["debug_callback"])) {
                            $options2["debug_callback"] =
                                $state["options"]["debug_callback"];
                            $options2["debug_callback_opts"] =
                                $state["options"]["debug_callback_opts"];
                        }
                        $state["proxyresponse"] = self::InitResponseState(
                            $state["fp"],
                            $state["debug"],
                            $options2,
                            $state["startts"],
                    ...
                    case "proxy_connect_response":
                        ...
                        $state["result"]["rawrecvsize"] +=
                            $result["rawrecvsize"];
                        $state["result"]["rawrecvheadersize"] +=
                            $result["rawrecvheadersize"];

                        if (substr($result["response"]["code"], 0, 1) != "2") {
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "Expected a 200 response from the CONNECT request.  Received:  %s.",
                                    $result["response"]["line"]
                                ),
                                "info" => $result,
                                "errorcode" => "proxy_connect_tunnel_failed",
                            ]);
                        }
                    ...
                    case "proxy_connect_enable_crypto":
                        ...
                        if ($result === false) {
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "A stream_socket_enable_crypto() failure occurred.  Most likely cause:  Tunnel connection failure or incompatible crypto setup."
                                ),
                                "errorcode" =>
                                    "stream_socket_enable_crypto_failed",
                            ]);
                    ...
                    case "send_data":
                        ...
                        // Queue up more data.
                        if (
                            isset($state["options"]["write_body_callback"]) &&
                            is_callable(
                                $state["options"]["write_body_callback"]
                            )
                        ) {
                            if (
                                $state["bodysize"] === false ||
                                $state["bodysize"] > 0
                            ) {
                                ...
                                if (
                                    ...
                                    return self::CleanupErrorState($state, [
                                        "success" => false,
                                        "error" => self::HTTPTranslate(
                                            "HTTP write body callback function failed."
                                        ),
                                ...
                                if ($state["bodysize"] === false) {
                                    ...
                                    $state["bodysize"] -= strlen(
                                        $state["data"]
                                    );
                                }
                            }
                        } elseif (
                            isset($state["options"]["files"]) &&
                            $state["bodysize"] > 0
                        ) {
                            // Select the next file to upload.
                            if (
                                $state["currentfile"] === false &&
                                count($state["options"]["files"])
                            ) {
                                $state["currentfile"] = array_shift(
                                    $state["options"]["files"]
                                );
                                ...
                                $filename = self::FilenameSafe(
                                    self::ExtractFilename(
                                        $state["currentfile"]["filename"]
                                    )
                                );
                                $type = self::HeaderValueCleanup(
                                ...
                                if (!isset($state["currentfile"]["datafile"])) {
                                    ...
                                    $state["currentfile"]["fp"] = @fopen(
                                        $state["currentfile"]["datafile"],
                                        "rb"
                                    );
                                    if ($state["currentfile"]["fp"] === false) {
                                        return self::CleanupErrorState($state, [
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        return self::CleanupErrorState($state, [
                                            "success" => false,
                                            "error" => self::HTTPTranslate(
                                                "A read error was encountered with the file '%s'.",
                                                $state["currentfile"][
                                                    "datafile"
                                                ]
                                            ),
                            ...
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "A weird internal HTTP error that should never, ever happen...just happened."
                                ),
            ...
            while ($state["state"] !== "done") {
                switch ($state["state"]) {
                    case "response_line":
                        ...
                        if ($pos === false) {
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "Unable to retrieve response line."
                                ),
                    ...
                    case "request_line":
                        ...
                        if ($pos === false) {
                            return self::CleanupErrorState($state, [
                                "success" => false,
                                "error" => self::HTTPTranslate(
                                    "Unable to retrieve request line."
                                ),
                                "errorcode" => "get_request_line",
                            ]);
                        }
                        $line = trim(substr($state["data"], 0, $pos));
                        $state["data"] = substr($state["data"], $pos + 1);
                    ...
                    case "body_chunked_headers":
                        ...
                        if (
                            !$result["success"] &&
                            ($state["state"] === "headers" ||
                                ($result["errorcode"] !== "stream_read_error" &&
                                    $result["errorcode"] !==
                                        "peer_disconnected"))
                        ) {
                            return self::CleanupErrorState($state, $result);
                        ...
                        if ($header != "") {
                            if (
                                ...
                                $state["lastheader"] = self::HeaderNameCleanup(
                                    substr($header, 0, $pos)
                                );
                                if (
                                    !isset(
                                ...
                                $state["result"]["headers"][
                                    $state["lastheader"]
                                ][] = ltrim(substr($header, $pos + 1));
                            }

                            $state["numheaders"]++;
                            if (
                                isset($state["options"]["maxheaders"]) &&
                                $state["numheaders"] >
                                    $state["options"]["maxheaders"]
                            ) {
                                return self::CleanupErrorState($state, [
                                    "success" => false,
                                    "error" => self::HTTPTranslate(
                                        "The number of headers exceeded the limit."
                                    ),
                                    "errorcode" => "headers_limit_exceeded",
                                ]);
                            }
                        } else {
                            if (
                                $state["result"]["response"]["code"] != 100 &&
                                isset(
                                    $state["options"]["read_headers_callback"]
                                ) &&
                                is_callable(
                                    $state["options"]["read_headers_callback"]
                                )
                                ...
                                if (
                                    ...
                                    return self::CleanupErrorState($state, [
                                        "success" => false,
                                        "error" => self::HTTPTranslate(
                                            "Read headers callback returned with a failure condition."
                                        ),
                            ...
                            if ($state["state"] === "body_chunked_headers") {
                                ...
                                if (
                                    $state["result"]["response"]["code"] ==
                                        100 ||
                                    $state["result"]["response"]["code"] == 101
                                ) {
                                    $state["state"] = "done";
                                    ...
                                    if (
                                        ...
                                        $state["autodecode"] = false;
                                    }
                                    if (!$state["autodecode"]) {
                                        $state["autodecode_ds"] = false;
                                    } else {
                                        if (
                                            !class_exists(
                                                "DeflateStream",
                                    ...
                                    if (
                                        ...
                                        $state["sizeleft"] = isset(
                                            ...
                                            : false;
                                        $state["state"] =
                                            !isset($state["proxyconnect"]) &&
                                            ($state["sizeleft"] !== false ||
                                                $state["client"])
                                                ? "body_content"
                                                : "done";
                                    }
                                    ...
                                    if (
                                        ...
                                        return [
                                            "success" => false,
                                            "error" => self::HTTPTranslate(
                                                "Intermission for adjustments to limits."
                                            ),
                                            "errorcode" => "no_data",
                                        ];
                                    }
                    ...
                    case "body_chunked_size":
                        ...
                        if ($size3 > 0) {
                            ...
                            if (
                                $state["result"]["response"]["code"] == 100 ||
                                !isset(
                                    $state["options"]["read_body_callback"]
                                ) ||
                                !is_callable(
                                    $state["options"]["read_body_callback"]
                                )
                ...
                if (
                    ...
                    $state["autodecode"] =
                        !isset($state["options"]["auto_decode"]) ||
                        $state["options"]["auto_decode"];
            ...
            if ($state["debug"]) {
                $state["result"]["rawrecv"] .= $state["rawdata"];
            }
            $state["result"]["rawrecvsize"] += $state["rawsize"];
            $state["result"]["rawrecvheadersize"] +=
                $state["rawrecvheadersize"];
            $state["result"]["endts"] = microtime(true);

            if (
                $state["close"] ||
                ($state["client"] &&
                    isset($state["result"]["headers"]["Connection"]) &&
                    strtolower($state["result"]["headers"]["Connection"][0]) ===
                        "close")
            ) {
                fclose($state["fp"]);
            } else {
    ...
    public static function RetrieveWebpage($url, $options = [])
    {
        ...
        if (
            ...
            return [
                "success" => false,
                "error" => self::HTTPTranslate(
                    "The functions 'stream_socket_client' and 'fsockopen' do not exist."
                ),
        ...
        if ($url["scheme"] != "http" && $url["scheme"] != "https") {
            return [
                "success" => false,
                "error" => self::HTTPTranslate(
                    "RetrieveWebpage() only supports the 'http' and 'https' protocols."
                ),
        ...
        $protocol =
            $secure && !$async
                ? (isset($options["protocol"])
                    ? strtolower($options["protocol"])
                    : "ssl")
                : "tcp";
        if (
            function_exists("stream_get_transports") &&
            !in_array($protocol, stream_get_transports())
        ) {
            return [
                "success" => false,
                "error" => self::HTTPTranslate(
        ...
        $useproxy =
            isset($options["proxyurl"]) && trim($options["proxyurl"]) != "";
        $proxysecure = false;
        $proxyconnect = false;
        $proxydata = "";
        if ($useproxy) {
            ...
            if ($proxysecure && $secure) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "The PHP SSL sockets implementation does not support tunneled SSL/TLS connections over SSL/TLS."
                    ),
            ...
            $proxyprotocol =
                $proxysecure && !$async
                    ? (isset($options["proxyprotocol"])
                        ? strtolower($options["proxyprotocol"])
                        : "ssl")
                    : "tcp";
            if (
                function_exists("stream_get_transports") &&
                !in_array($proxyprotocol, stream_get_transports())
            ) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "The desired transport proxy protocol '%s' is not installed.",
                        $proxyprotocol
                    ),
                    "errorcode" => "proxy_transport_not_installed",
                ];
            }
            $proxyhost = str_replace(
                " ",
                "-",
                self::HeaderValueCleanup($proxyurl["host"])
            );
            if ($proxyhost === "") {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "The specified proxy URL is not a URL.  Prefix 'proxyurl' with http:// or https://"
                    ),
                    "errorcode" => "invalid_proxy_url",
                ];
            }
            $proxyport = (int) $proxyurl["port"]
                ? (int) $proxyurl["port"]
                : ($proxysecure
                    ? 443
                    : 80);
            ...
            $proxyconnect =
                isset($options["proxyconnect"]) && $options["proxyconnect"]
            ...
            if ($proxyconnect) {
                ...
                $options["proxyheaders"] = self::NormalizeHeaders(
                    $options["proxyheaders"]
                );
                if (isset($options["rawproxyheaders"])) {
                    self::MergeRawHeaders(
                        $options["proxyheaders"],
                        $options["rawproxyheaders"]
                    );
                }
                ...
                $proxydata .= "\r\n";
                if (
                    isset($options["debug_callback"]) &&
                    is_callable($options["debug_callback"])
                ) {
                    call_user_func_array($options["debug_callback"], [
                        "rawproxyheaders",
                        $proxydata,
        ...
        if (!isset($options["method"])) {
            if (
                (isset($options["write_body_callback"]) &&
                    is_callable($options["write_body_callback"])) ||
                isset($options["body"])
            ) {
                $options["method"] = "PUT";
            } elseif (
                isset($options["postvars"]) ||
                (isset($options["files"]) && count($options["files"]))
            ) {
                $options["method"] = "POST";
            } else {
                $options["method"] = "GET";
        ...
        if (
            ...
            call_user_func_array($options["write_body_callback"], [
                &$body,
                &$bodysize,
                &$options["write_body_callback_opts"],
            ]);
            ...
            $data .=
                "Content-Type: multipart/form-data; boundary=" . $mime . "\r\n";
                    ...
                    if (!is_array($val)) {
                        if (is_string($val) || is_numeric($val)) {
                            ...
                            return [
                                "success" => false,
                                "error" =>
                                    "A supplied 'postvars' value is an invalid type.  Expected string, numeric, or array.",
                                "errorcode" => "invalid_postvars_value",
                                "info" => ["name" => $name, "val" => $val],
                            ];
                        }
                    }
            ...
            foreach ($options["files"] as $num => $info) {
                ...
                $name = str_replace("\"", "", $name);
                $filename = self::FilenameSafe(
                    self::ExtractFilename($info["filename"])
                );
                ...
                $info["filesize"] = isset($info["datafile"])
                    ? self::RawFileSize($info["datafile"])
                    : strlen($info["data"]);
        ...
        if (
            ($bodysize === false && strlen($body) > 0) ||
            ($bodysize !== false && $bodysize < strlen($body))
        ) {
            $bodysize = strlen($body);
        }
        ...
        if (isset($options["fp"]) && is_resource($options["fp"])) {
            ...
            if ($timeleft !== false) {
                $options["proxyconnecttimeout"] = min(
                    $options["proxyconnecttimeout"],
                    $timeleft
                );
            }
            if (!function_exists("stream_socket_client")) {
                ...
                if (isset($options["source_ip"])) {
                    $context["socket"] = [
                        "bindto" => $options["source_ip"] . ":0",
                    ];
                }
                if ($proxysecure) {
                    if (
                        !isset($options["proxysslopts"]) ||
                        !is_array($options["proxysslopts"])
                    ) {
                    ...
                    if (
                        !isset($options["sslopts"]) ||
                        !is_array($options["sslopts"])
                    ) {
            ...
            if ($fp === false) {
                return [
                    "success" => false,
                    "error" => self::HTTPTranslate(
                        "Unable to establish a connection to '%s'.",
                        ($proxysecure ? $proxyprotocol . "://" : "") .
                            $proxyhost .
                            ":" .
                            $proxyport
                    ),
