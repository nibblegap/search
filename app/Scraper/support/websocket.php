...
class WebSocket
{
    ...
    public function Connect($url, $origin, $options = [], $web = false)
    {
        $this->Disconnect();

        if (class_exists("CSPRNG", false) && $this->csprng === false) {
            $this->csprng = new CSPRNG();
        }

        if (
            isset($options["connected_fp"]) &&
            is_resource($options["connected_fp"])
        ) {
            $this->fp = $options["connected_fp"];
        } else {
            ...
            if ($url2["scheme"] != "ws" && $url2["scheme"] != "wss") {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "WebSocket::Connect() only supports the 'ws' and 'wss' protocols."
                    ),
            ...
            if ($result["response"]["code"] != 101) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "WebSocket::Connect() failed to connect to the WebSocket.  Server returned:  %s %s",
                        $result["response"]["code"],
                        $result["response"]["meaning"]
                    ),
                    "errorcode" => "incorrect_server_response",
                ];
            }
            if (!isset($result["headers"]["Sec-Websocket-Accept"])) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "Server failed to include a 'Sec-WebSocket-Accept' header in its response to the request."
                    ),
                    "errorcode" => "missing_server_websocket_accept_header",
                ];
            }

            // Verify the Sec-WebSocket-Accept response.
            if (
                $result["headers"]["Sec-Websocket-Accept"][0] !==
                base64_encode(sha1($key . self::KEY_GUID, true))
            ) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "The server's 'Sec-WebSocket-Accept' header is invalid."
                    ),
                    "errorcode" => "invalid_server_websocket_accept_header",
                ];
            }
    ...
    public function Disconnect()
    {
        if ($this->fp !== false && $this->state === self::STATE_OPEN) {
            if ($this->closemode === self::CLOSE_IMMEDIATELY) {
                $this->writemessages = [];
            } elseif ($this->closemode === self::CLOSE_AFTER_CURRENT_MESSAGE) {
            ...
            $this->Write(
                "",
                self::FRAMETYPE_CONNECTION_CLOSE,
                true,
                $this->client
            );
    ...
    {
        if ($this->fp === false || $this->state === self::STATE_CONNECTING) {
            return [
                "success" => false,
                "error" => self::WSTranslate("Connection not established."),
        ...
        if ($wait) {
            while (
                !count($this->readmessages) ||
                ($finished && !$this->readmessages[0]["fin"])
            ) {
                $result = $this->Wait();
                if (!$result["success"]) {
                    return $result;
        ...
        if (count($this->readmessages)) {
            if ($finished) {
                if ($this->readmessages[0]["fin"]) {
                    $data = array_shift($this->readmessages);
                }
            } else {
    ...
    public function Write(
        ...
        if ($frametype >= 0x08 || $lastcompleted) {
            ...
            array_splice($this->writemessages, $pos !== false ? $pos : $y, 0, [
                $frame,
            ]);
        } else {
            if ($frametype !== $this->writemessages[$y - 1]["opcode"]) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "Mismatched frame type (opcode) specified."
                    ),
                    "errorcode" => "mismatched_frame_type",
                ];
            }
    ...
    public function Wait($timeout = false)
    {
        ...
        $readfp = [$this->fp];
        $writefp = $this->writedata !== "" ? [$this->fp] : null;
        $exceptfp = null;
        if ($timeout === false || $timeout > $this->keepalive) {
        ...
        if ($result === false) {
            return [
                "success" => false,
                "error" => self::WSTranslate(
                    "Wait() failed due to stream_select() failure.  Most likely cause:  Connection failure."
                ),
        ...
        $result = $this->ProcessQueuesAndTimeoutState(
            $result > 0 && count($readfp),
            $result > 0 && $writefp !== null && count($writefp)
        );
    ...
    public function ProcessQueuesAndTimeoutState(
        $read,
        $write,
        $readsize = 65536
    ) {
        ...
        if ($read) {
            ...
            if ($result === false || ($result === "" && feof($this->fp))) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "ProcessQueuesAndTimeoutState() failed due to fread() failure.  Most likely cause:  Connection failure."
                    ),
            ...
            if ($result !== "") {
                ...
                if (
                    $this->maxreadframesize !== false &&
                    strlen($this->readdata) > $this->maxreadframesize
                ) {
                    return [
                        "success" => false,
                        "error" => self::WSTranslate(
                            "ProcessQueuesAndTimeoutState() failed due to peer sending a single frame exceeding %s bytes of data.",
                            $this->maxreadframesize
                        ),
                        "errorcode" => "max_read_frame_size_exceeded",
                    ];
                }
        ...
        if ($write) {
            ...
            if (
                $result === false ||
                ($this->writedata === "" && feof($this->fp))
            ) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "ProcessQueuesAndTimeoutState() failed due to fwrite() failure.  Most likely cause:  Connection failure."
                    ),
        ...
        if ($this->lastkeepalive < time() - $this->keepalive) {
            if ($this->keepalivesent) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "ProcessQueuesAndTimeoutState() failed due to non-response from peer to ping frame.  Most likely cause:  Connection failure."
                    ),
    ...
    protected function ProcessReadData()
    {
        while (($frame = $this->ReadFrame()) !== false) {
            ...
            if (
                ($frame["opcode"] >= 0x03 && $frame["opcode"] <= 0x07) ||
                $frame["opcode"] >= 0x0b
            ) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "Invalid frame detected.  Bad opcode 0x%02X.",
                        $frame["opcode"]
                    ),
                    "errorcode" => "bad_frame_opcode",
                ];
            }
            ...
            if ($frame["rsv1"] || $frame["rsv2"] || $frame["rsv3"]) {
                return [
                    "success" => false,
                    "error" => self::WSTranslate(
                        "Invalid frame detected.  One or more reserved extension bits are set."
                    ),
                    "errorcode" => "bad_reserved_bits_set",
                ];
            }
            ...
            if ($frame["opcode"] >= 0x08) {
                ...
                if (!$frame["fin"]) {
                    return [
                        "success" => false,
                        "error" => self::WSTranslate(
                            "Invalid frame detected.  Fragmented control frame was received."
                        ),
                        "errorcode" => "bad_control_frame",
                    ];
                }
                ...
                if ($frame["opcode"] === self::FRAMETYPE_CONNECTION_CLOSE) {
                    if ($this->state === self::STATE_CLOSE) {
                        ...
                        if ($this->closemode === self::CLOSE_IMMEDIATELY) {
                            ...
                            $this->closemode ===
                            self::CLOSE_AFTER_CURRENT_MESSAGE
                        ) {
                            $this->writemessages = array_slice(
                                $this->writemessages,
                                0,
                                1
                            );
                ...
                } elseif ($frame["opcode"] === self::FRAMETYPE_PING) {
                    if ($this->state !== self::STATE_CLOSE) {
                ...
                } elseif ($frame["opcode"] === self::FRAMETYPE_PONG) {
                    // Do nothing.
                }
            } else {
                ...
                $lastcompleted =
                    !count($this->readmessages) ||
                    $this->readmessages[count($this->readmessages) - 1]["fin"];
                if ($lastcompleted) {
                    ...
                    if ($frame["opcode"] === self::FRAMETYPE_CONTINUATION) {
                        return [
                            "success" => false,
                            "error" => self::WSTranslate(
                                "Invalid frame detected.  Fragment continuation frame was received at the start of a fragment."
                            ),
                            "errorcode" => "bad_continuation_frame",
                        ];
                    }
                    ...
                    if ($frame["opcode"] !== self::FRAMETYPE_CONTINUATION) {
                        return [
                            "success" => false,
                            "error" => self::WSTranslate(
                                "Invalid frame detected.  Fragment continuation frame was not received for a fragment."
                            ),
                            "errorcode" => "missing_continuation_frame",
                        ];
                    }

                    $this->readmessages[count($this->readmessages) - 1]["fin"] =
                        $frame["fin"];
                    $this->readmessages[count($this->readmessages) - 1][
                        "payload"
                    ] .= $frame["payload"];
                }

                if (
                    $this->maxreadmessagesize !== false &&
                    strlen(
                        $this->readmessages[count($this->readmessages) - 1][
                            "payload"
                        ]
                    ) > $this->maxreadmessagesize
                ) {
                    return [
                        "success" => false,
                        "error" => self::WSTranslate(
                            "Peer sent a single message exceeding %s bytes of data.",
                            $this->maxreadmessagesize
                        ),
                        "errorcode" => "max_read_message_size_exceeded",
                    ];
                }
            }
    ...
    protected function ReadFrame()
    {
        ...
        if (strlen($this->readdata) < $start + ($mask ? 4 : 0)) {
            return false;
        }

        // Frame minus payload calculated.
        if ($length == 126) {
            $length = self::UnpackInt(substr($this->readdata, 2, 2));
        } elseif ($length == 127) {
            $length = self::UnpackInt(substr($this->readdata, 2, 8));
        }
        ...
        if (strlen($this->readdata) < $start + $length) {
            return false;
        }
        ...
        if ($mask) {
            ...
            for ($x = 0; $x < $length; $x++) {
                $payload[$x] = chr(
                    ord($payload[$x]) ^ ord($maskingkey[$x % 4])
                );
            }
        }
    ...
    {
        while (
            strlen($this->writedata) < 65536 &&
            count($this->writemessages) &&
            count($this->writemessages[0]["payloads"])
        ) {
            $payload = array_shift($this->writemessages[0]["payloads"]);

            $fin =
                $this->writemessages[0]["fin"] &&
                !count($this->writemessages[0]["payloads"]);

            if ($this->writemessages[0]["framessent"] === 0) {
                $opcode = $this->writemessages[0]["opcode"];
            } else {
                $opcode = self::FRAMETYPE_CONTINUATION;
            }
            ...
            if ($fin) {
                array_shift($this->writemessages);
            }
        }
    }
    ...
    protected function WriteFrame($fin, $opcode, $payload)
    {
        ...
        if ($length === 126) {
            ...
            $data .= self::PackInt64(strlen($payload));
        }
    ...
    {
        if ($this->csprng !== false) {
            $result = $this->csprng->GetBytes($length);
        } else {
            $result = "";
    ...
    public static function PackInt64($num)
    {
        ...
        while (is_float($num)) {
            ...
            $num = floor($num / 256);
            if (is_float($num) && $num < (float) $floatlim) {
                $num = (int) $num;
            }
        }
