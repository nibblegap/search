...
class CRC32Stream
{
    ...
    {
        if ($options === false && function_exists("hash_init")) {
    ...
    public function AddData($data)
    {
        ...
        if ($this->hash !== false) {
            ...
            for ($x = 0; $x < $y; $x++) {
                if ($this->datareflect) {
                    $this->currcrc =
                        $this->SHL32($this->currcrc, 8) ^
                        $this->crctable[
                            $this->SHR32($this->currcrc, 24) ^
                                self::$revlookup[ord($data[$x])]
                        ];
                } else {
                    $this->currcrc =
                        $this->SHL32($this->currcrc, 8) ^
                        $this->crctable[
                            $this->SHR32($this->currcrc, 24) ^ ord($data[$x])
                        ];
                }
            }
        }
    ...
    public function Finalize()
    {
        ...
        if ($this->hash !== false) {
            ...
            if ($this->crcreflect) {
                ...
                $this->currcrc =
                    self::$revlookup[$this->SHR32($tempcrc, 24)] |
                    $this->SHL32(
                        self::$revlookup[$this->SHR32($tempcrc, 16) & 0xff],
                        8
                    ) |
                    $this->SHL32(
                        self::$revlookup[$this->SHR32($tempcrc, 8) & 0xff],
                        16
                    ) |
                    $this->SHL32(
                        self::$revlookup[$this->LIM32($tempcrc & 0xff)],
                        24
                    );
            }
